...
class MaterialTriangle : public Triangle {
 public:
  ...
  void SetSpecular(float3 color, float shininess) {
    specular_color = color;
    specular_exponent = shininess;
  };
  void SetReflectiveness(bool reflective) { reflectiveness = reflective; };
  void SetReflectivenessAndTransparency(bool reflective_and_transparent) {
    reflectiveness_and_transparency = reflective_and_transparent;
  };
...
class LightingAndShadows : public MTAlgorithm {
 ...
 protected:
  ...
  virtual Payload Hit(const Ray& ray, const IntersectableData& data,
                      const MaterialTriangle* traingle,
                      const unsigned int max_raytrace_depth) const;
