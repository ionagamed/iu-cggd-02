...
class IntersectableData {
 public:
  ...
  IntersectableData(float t, float3 baricentric)
      : t(t), baricentric(baricentric){};
  ~IntersectableData(){};
  float t;
  float3 baricentric;
};
...
class Vertex {
 public:
  ...
  Vertex(float3 position, float3 normal, float3 tex)
      : position(position), tex(tex), color(float3{1.0, 1.0, 1.0}) {
    this->normal = normalize(normal);
  };
  Vertex(float3 position, float3 normal, float3 tex, float3 color)
      : position(position), tex(tex), color(color) {
    this->normal = normalize(normal);
  };
...
class MTAlgorithm : public RayGenerationApp {
 ...
 protected:
  virtual Payload TraceRay(const Ray& ray,
